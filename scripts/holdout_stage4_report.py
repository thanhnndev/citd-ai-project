...
def sensitivity_section(sensitivity: dict, sensitivity_path: Path) -> str:
    ...
    return (
        ...
        + table
        + "\n"
        + "Trên máy này, `thread_count` **có** làm thay đổi kết quả: đổi cấu hình làm "
        ...
        + "`tc=2` và `tc=-1` so với `tc=1`), kéo theo chọn top 50% và net R đổi. "
        + "Hai lần chạy cùng cấu hình `tc=1` cho prediction giống hệt nhau "
        ...
        + "Ảnh hưởng nằm ở bước train: cùng model đã fit, đổi prediction `thread_count` 1/2/-1 cho "
        + "prediction giống hệt nhau (max|Δp| = 0.0). CatBoost mô tả `thread_count` là tham số tốc độ "
        + "không ảnh hưởng kết quả; phép đo trên máy này xác nhận điều đó ở bước prediction nhưng "
        + "không xác nhận ở bước train (holdout ROC-AUC từ "
